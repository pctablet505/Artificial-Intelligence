...
def perceptron(data, labels, params={}, hook=None):
    ...
    count_mistakes = 0
    for _ in range(T):
        old_count_mistakes = count_mistakes
        for i in range(n):
            x = data[:, i:i + 1]
            y = labels[i][0]

            prediction = (theta.transpose() @ x) + theta_0

            if np.sign(y * prediction) <= 0:
                theta = theta + y * x
                theta_0 = theta_0 + y
                count_mistakes += 1
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    for _ in range(T):
        ...
        for i in range(n):
            ...
            hist_theta_0.append(theta_0)

    return (np.average(hist_theta, axis=0), np.average(hist_theta_0, axis=0))
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    no_of_test_data_instances = labels_test.shape[1]
    theta, theta_0 = learner(data_train, labels_train)
    score_calculated = score(data_test, labels_test, theta, theta_0)
    return score_calculated / no_of_test_data_instances
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for _ in range(it):
        ...
        sum_scores += eval_classifier(learner, data_train, labels_train, data_test, labels_test)

    return sum_scores / it
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for j in range(k):
        D_j = data_split[j]
        labels_j = labels_split[j]

        D_minus_j = np.concatenate(data_split[:j] + data_split[j + 1:], axis=1)
        labels_minus_j = np.concatenate(labels_split[:j] + labels_split[j + 1:], axis=1)

        score_sum += eval_classifier(learner, D_minus_j, labels_minus_j, D_j, labels_j)
...
s=0
for i in range(100):
    s+=eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5)
print(s/100)
'''
