...
class Space():
    ...
    def available_spaces(self):
        ...
        candidates = set(
            (row, col) for row in range(self.height) for col in range(self.height)
        )
        # Remove all houses and hospitals
        candidates = candidates - self.houses
        candidates = candidates - self.hospitals
        return candidates
    ...
    def hill_climb(self, maximum=None, image_prefix=None, log=False):
        ...
        self.hospitals = set()
        available_spaces = list(self.available_spaces())
        random.shuffle(available_spaces)
        self.hospitals.update(set(available_spaces[:min(self.num_hospitals, len(available_spaces))]))
        if log:
            print('Initial state: cost', self.get_cost(self.available_spaces()))
        if image_prefix:
            self.output_image(f'{image_prefix}{str(count).zfill(3)}.png')

        # Continue untill we reach maximum number of iterations
        while maximum is None or count < maximum:
            ...
            for hospital in self.hospitals:
                # consider all neighbours for that hospital
                for replacement in self.get_neighbors(*hospital):
                    # generate a neighbouring set of hospitals
            ...
            # None of the neighbours are better tha current state
            if best_neighbor_cost >= self.get_cost(self.hospitals):
                return self.hospitals

            # Move to a highest_valued Neighbour
            else:
                if log:
    ...
    def random_restart(self, maximum, image_prefix=None, log=False):
        ...
        # Repaeat hill-climbing for a fixed number of times
    ...
    def get_cost(self, hospitals):
        ...
        for house in self.houses:
            ...
            for hospital in hospitals:
                mc=min(mc,abs(house[0] - hospital[0]) + abs(house[1] - hospital[1]))
            cost+=mc
        return cost
...
# Ceate a new space and add houses randomly
s = Space(height=10, width=10, num_hospitals=10)
for i in range(10):
