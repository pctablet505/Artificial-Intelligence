"""Games of Adverserial Search (Chapter 5)"""
...
def minmax_decision(state, game):
    ...
    return max(game.action(state), key=lambda a: min_value(game.result(state, a)))
...
def expect_minmax(game, state):
    ...
    def chance_node(state, action):
        ...
        if game.terminal_state(res_state):
        ...
        for chance in game.chances(res_state):
            ...
            sum_chances += util * game.porbability(chance)
...
def alpha_beta_search(state, game):
    ...
    def max_value(state, alpha, beta):
        ...
        v = -np.inf
        for a in game.actions(state):
            v = max(v, min(game.result(state, a), alpha, beta))
            if v >= beta:
                return v
            alpha = max(alpha, v)
        return v
    ...
    def min_value(state, alpha, beta):
        ...
        v = np.inf
        for a in game.actions(state):
            v = min(v, max(game.result(state, a), alpha, beta))
            if v <= alpha:
                return v
            beta = min(beta, v)
        return v
...
def monte_carlo_tree_search(state, game, N=1000):
    ...
    def simulate(game, state):
        """Simulate the utility f current state by randomly picking a step."""
        player = game.to_move(state)
...
class Game:
    """A game is similar to a problem, but it has a utility for each
     state and a terminal test instead of a path cost and a goal test.
     To create a game, subclass this class and implement actions,
     result, utility, and terminal_test. You may override display and
     successors or you can inherit their default methods. You will
     also need to set the .initial attribute to the initial state; this
     can be done in the constructor."""

    def actions(self, state):
        """Return a list of legal moves at this point."""
...
class StochasticGame(Game):
    ...
    def outcome(self, state, chance):
        """Return the state which is outcome of a chance trail."""
        raise NotImplementedError
    ...
    def play_game(self, *players):
        ...
        while True:
            for player in players:
                ...
                move = player(self, state)
                state = self.result(state, move)
                if self.terminal_test(state, move):
                    self.display(state)
                    return self.utility(state, self.to_move(self.initial))
...
class Backgammon(StochasticGame):
    ...
    def compute_utility(self, board, move, player):
        """If 'W' wins with this move, return 1; if 'B' wins return -1; els return 0."""
        util = {'W': 1, 'B': -1}
    ...
    def is_legal_move(self, board, start, steps, player):
        """Move is a tuple which contains starting points of checker to be
                moved during a player's turn. An on-game board move is legal if  both the
                destinations are open. A bear-off move is the one where a checker is
                moved off-board.
        It is legal only after a player has moved all hsi checkers to his home.
        """
