...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap:
            ...
            for X in self.domains[x]:
                overlap_possibility = False
                for Y in self.domains[y]:
                    if X != Y and X[a] == Y[b]:
                        overlap_possibility = True
                        break
                if not overlap_possibility:
                    domains_to_remove.add(X)
    ...
    def ac3(self, arcs=None):
        ...
        queue = deque()
        if arcs is None:
            for variable_1 in self.crossword.variables:
                for variable_2 in self.crossword.neighbors(variable_1):
        ...
        for a in assignment:
            ...
            for b in assignment:
                ...
                if a != b:
                    if w1 == w2:
                        return False
                    overlap = self.crossword.overlaps[a, b]
                    if overlap:
                        x, y = overlap
                        if w1[x] != w2[y]:
                            return False
            ...
            for var2 in neighbors:
                for value2 in self.domains[var2]:
                    ...
                    if overlap:
                        ...
                        if value[a] != value2[b]:
                            count_ruled_out += 1
            result.append([value, count_ruled_out])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        if variables_list:
            return min(variables_list, key=lambda x: (x[1], -1 * x[2]))[0]
        return None
