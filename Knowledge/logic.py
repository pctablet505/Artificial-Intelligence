...
class And(Sentence):
    ...
    def __hash__(self):
        return hash('and', tuple(hash(conjuct) for conjuct in self.conjucts))

    def __repr__(self):
        conjuctions = ', '.join([str(conjuct) for conjuct in self.conjucts])
        return f'And({conjuctions})'

    def add(self, conjuct):
        Sentence.validate(conjuct)
        self.conjucts.append(conjuct)

    def evaluate(self, model):
        return all(conjuct.evaluate(model) for conjuct in self.conjucts)

    def formula(self):
        if len(self.conjucts) == 1:
            return self.conjucts[0].formula()
        return ' ^ '.join([
            Sentence.parenthesize(
                conjuct.formula()) for conjuct in self.conjucts])

    def symbols(self):
        return set.union(*[conjuct.symbols() for conjuct in self.conjucts])
...
class Or(Sentence):
    ...
    def __hash__(self):
        return hash('and', tuple(hash(disjunct) for disjunct in self.disjuncts))
...
def model_check(knowledge, query):
    ...
    def check_all(knowledge, query, symbols, model):
        '''Checks if knowledge base entails query,
        given a particular modle.'''

        # If modlel has an assignment for each symbol
        if not symbols:
            # If knowledge base is true in modlel, then query must also be true
